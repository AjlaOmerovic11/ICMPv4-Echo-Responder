...
U prvom scenariju ispitan je ispravan rad modula icmp_echo_responder u slučaju prijema validnog ICMPv4 Echo Request paketa, sa posebnim fokusom na verifikaciju ready/valid handshake mehanizma u scenariju u kojem se generiše ICMP Echo Reply odgovor. Pristigli Ethernet okvir sadrži ispravne odredišne MAC i IPv4 adrese definisane generičkim parametrima modula. Nakon uspješnog parsiranja Ethernet i IPv4 zaglavlja, modul prepoznaje ICMP Echo Request poruku i prelazi u fazu generisanja ICMP Echo Reply paketa. U ovom koreku se vrši zamjena izvornih i odredišnih MAC i IP adresa.
Na ulaznom interfejsu, modul kontinuirano prihvata dolazne bajtove dok su signali in_valid i in_ready istovremeno aktivni.
Verifikacija ready/valid handshake mehanizma je realizovana na izlaznom interfejsu, gdje se signal out_valid aktivira tek nakon što započine ICMP Echo Reply. Prenos izlaznih bajtova vrši se isključivo u taktovima kada je i signal out_ready aktivan. U slučajevima kada je out_ready privremeno deaktiviran modul zadržava trenutni izlazni bajt (0B) stabilnim i ostaje u stanju REPLY, bez promjene FSM stanja i bez gubitka podataka. Nakon ponovne aktivacije out_ready, slanje ICMP Echo Reply paketa se nastavlja tačno od prekinute pozicije, čime je potvrđena korektna implementacija ready/valid handshake mehanizma.
Po završetku slanja kompletnog ICMP Echo Reply paketa, modul se ispravno vraća u početno IDLE stanje i spreman je za obradu narednih Ethernet okvira.

U narednim verifikacionim scenarijima, kada modul detektuje neispravne pakete i prelazi u IGNORE stanje, ready/valid handshake mehanizam na izlazu se ne testira. U tom slučaju ne dolazi do generisanja ICMP Echo Reply odgovora (out_valid ostaje neaktivan sve vrijeme). Time se jasno razdvaja verifikacija kontrole toka podataka u scenarijima sa odgovorom od scenarija u kojima se paket isključivo odbacuje. Po završetku slanja ICMP Echo Reply paketa, modul se ispravno vraća u početno stanje i spreman je za obradu narednih Ethernet okvira.
